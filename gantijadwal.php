...
<meta http-equiv="refresh" content="3;url=gantijadwal.php">
				...
				if (count($bagi) == 6) {
					...
					if ($pin == $jabatan[0]['pin'] && $jabatan[0]['jabatan'] == "Dosen") {
						if (strtolower($bagi[0]) == "ch") {
							...
							if (!$dtmtkul > 0) {
								$replyPesan .= "Maaf kode mata kuliah atau format anda salah";
								$ide = $list['ID'];
							}else{
								if ($cekjk[0]['kd_dm'] == NULL) {
									//$crud->jadwal("tjadwal","kd_dm = 'NULL'", "hari = '$hari' AND kd_dm = '$kd_dm' AND kelas = '$kelas' AND semester = '$semester' AND jurusan = '$kd_jurusan'");
									$crud->jadwal("tjadwal","kd_dm = '$kd_dm'", "hari = '$hari' AND jam = '$jam' AND kelas = '$kelas' AND semester = '$semester' AND jurusan = '$kd_jurusan'");
									$ide = $list['ID'];
									$data = $crud->fetch("tkrs tk LEFT JOIN tpengguna tp ON tk.no_reg = tp.no_reg","tk.kd_dm = '$kd_dm' AND tp.kelas = '$kelas' AND tp.thn_akademik = '$ang' AND tp.jurusan = '$jurusan'");
									print_r($data);
